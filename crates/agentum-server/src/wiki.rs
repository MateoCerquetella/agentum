...
//! — a garbled/empty index must NOT pass, so an inconclusive run never shows as
//! a success), the slug guard, and the generation-prompt builder. The HTTP surface
...
pub(crate) fn build_wiki_prompt(
    ...
    p.push_str(
        "You are Agentum's AutoWiki generator. Produce a structured, \
...
mod tests {
    ...
    fn wiki_key_collapses_checkouts_of_the_same_remote() {
        ...
        let ssh = wiki_key(Some("git@github.com:owner/repo"), "/home/tester/remote");
        assert_eq!(https, ssh);
