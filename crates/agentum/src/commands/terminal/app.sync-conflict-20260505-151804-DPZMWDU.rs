        ...
        KeyCode::Enter => {
                ...
                .await
            {
                Ok(created) => {
                    ...
                    switch_to_session(app, id, client, term_tx, stream_handle);
                    return;
                }
                Err(e) => {
...
/// Force-attach the terminal stream to `id`, regardless of what `app.selected`
/// currently points at. Used right after creating a new session: the form
/// needs the pane to start streaming immediately rather than waiting for the
/// user to navigate, and `update_selection` would short-circuit because
/// `app.selected` was already mutated to the new id.
