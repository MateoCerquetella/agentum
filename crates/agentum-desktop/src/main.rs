...
//!
//! Why Tauri (not raw wry/tao): Tauri gives us cross-platform installers, an
//! auto-updater, and native menu/tray/notifications (the latter wired up in
//! Phase D) for roughly the same footprint, since it sits on the same
//! wry/webkit stack we were already linking.
...
fn main() -> Result<()> {
    ...
    // Owned by `main` so it outlives Tauri's blocking `run()` — dropping it
    // early would tear the daemon down under the window.
    ...
    // The window URL is dynamic (the loopback port is chosen at runtime), so we
    // can't declare it in tauri.conf.json — build it in the setup hook instead.
