...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'agentum::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'agentum',
      repoName: 'AGENTUM',
      cpu: 1.5,
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      { id: 'agentum::/remote/Stingray@@abcd1234', cwd: '', title: 'agentum/Stingray' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['agentum', 'ssh-conn-1']])
    })
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'agentum::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'agentum::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      { id: 'agentum::/local/Triton@@deadbeef', cwd: '/local/Triton', title: 'agentum/Triton' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['agentum', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'agentum',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    const tabId = 'tab-xyz'
    const ds: DaemonSession[] = [{ id: 'agentum::/wrong/path@@feedface', cwd: '', title: 'agentum' }]
    const ctx = baseCtx({
      tabsByWorktree: {
        'agentum::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['agentum::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('agentum::/correct/path')
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'agentum::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('remote-orphan interaction state: null metrics + bound=false', () => {
    const ds: DaemonSession[] = [{ id: 'agentum::/remote/Wt@@deadbeef', cwd: '', title: 'agentum/Wt' }]
    const out = mergeSnapshotAndSessions(null, ds, baseCtx())
    const session = out[0].worktrees[0].sessions[0]
    expect(session).toMatchObject({
      hasLocalSamples: false,
