...
export const AGENTUM_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'agentum:editor-external-file-change'
export const AGENTUM_EDITOR_SAVE_FILE_EVENT = 'agentum:editor-save-file'
export const AGENTUM_EDITOR_SAVE_AND_CLOSE_EVENT = 'agentum:save-and-close'
export const AGENTUM_EDITOR_FILE_SAVED_EVENT = 'agentum:editor-file-saved'
export const AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT = 'agentum:editor-request-cmd-save'
export const AGENTUM_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'agentum:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(AGENTUM_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(AGENTUM_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(AGENTUM_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(AGENTUM_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
