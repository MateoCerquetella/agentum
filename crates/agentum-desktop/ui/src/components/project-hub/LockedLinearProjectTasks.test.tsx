...
describe('locked Linear board', () => {
  ...
  it('authors a repo-owned spec from an exact hydrated Linear issue', () => { const source = readFileSync(new URL('./LockedLinearProjectTasks.tsx', import.meta.url), 'utf8'); expect(source).toContain('linearGetIssue(settings, issue.id, scope.workspaceId)'); expect(source).toContain('requestNewSpecFromWorkItem({'); expect(source).toContain('repoId: scope.repoId'); expect(source).toContain("provider: 'linear'"); expect(source).toContain('New Spec'); expect(source).not.toContain('new-workspace-composer') })
})
