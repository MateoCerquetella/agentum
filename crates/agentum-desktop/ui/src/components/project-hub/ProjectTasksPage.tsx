...
export function ProjectTasksPage({ repo }: { repo: Repo }): React.JSX.Element {
  ...
  if (scope.status === 'loading') return <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Resolving {repo.displayName}&apos;s task board…</div>
  if (scope.status === 'unbound' || scope.status === 'unavailable') return <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center"><p className="text-sm font-medium">{repo.displayName} has no available project board.</p>{scope.status === 'unavailable' ? <p className="max-w-lg text-xs text-muted-foreground">{scope.message}</p> : <p className="text-xs text-muted-foreground">Choose a provider and exact board in Project Settings.</p>}<Button size="sm" onClick={openSettings}>Open Project Settings</Button></div>
  return <div className="flex h-full min-h-0 flex-col">
    <details className="flex-none border-b border-border/60">
      ...
      <div className="max-h-[55vh] overflow-auto border-t border-border/50 p-4">
        <div className="mb-3 flex items-center justify-between gap-3 text-xs text-muted-foreground"><span>Read-only binding summary. Change this board in Project Settings.</span><Button size="sm" variant="outline" onClick={openSettings}>Open Project Settings</Button></div>
