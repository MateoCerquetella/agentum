...
export const CLOSE_ALL_CONTEXT_MENUS_EVENT = 'agentum-close-all-context-menus'
...
export default function SortableTab({
  ...
  const tabRoot = (
    <div
      ...
      className={`group relative flex items-center h-full px-1.5 text-xs cursor-pointer select-none shrink-0 outline-none focus:outline-none focus-visible:outline-none border-t ${hasTabsToRight ? 'border-r' : ''} border-border ${isActive ? 'bg-card' : 'bg-sidebar'} ${getDropIndicatorClasses(dropIndicator ?? null)} ${
    ...
    >
      ...
      {showActivityAffordance ? (
        ...
        // Agentum's filled bell glyph (amber-500 with a subtle drop shadow)
        // so it matches the worktree-level bell in the sidebar — keeping
        // every "needs your attention" surface in Agentum consistent.
