...
import {
  ...
  AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT,
...
function Terminal(): React.JSX.Element | null {
  ...
  const terminalShortcutPolicy = useAppStore(
    (s) => s.settings?.terminalShortcutPolicy ?? 'agentum-first'
  )
  ...
  // the same row as the "Agentum" title. The target element is created by App.tsx.
  ...
  // AGENTUM_EDITOR_REQUEST_FILE_CLOSE_EVENT) must not re-open the dialog over
  ...
  const handleSaveDialogSave = useCallback(async () => {
    ...
    setSaveDialogFileId(null)
    window.dispatchEvent(new CustomEvent(AGENTUM_EDITOR_SAVE_AND_CLOSE_EVENT, { detail: { fileId } }))
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (context !== 'terminal' || terminalShortcutPolicy !== 'agentum-first') {
          return
        }
      ...
      if (!e.repeat && matchShortcut('editor.save')) {
        ...
        if (!inEditor) {
          ...
          if (state.activeTabType === 'editor' && state.activeFileId) {
            ...
            window.dispatchEvent(new Event(AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT))
            return
          }
        }
      }
  ...
  return (
    ...
    >
      ...
      {activeView === 'terminal' && activeWorktree ? (
        <SddWorkspaceBar repoId={activeWorktree.repoId} projectName={activeWorktree.displayName} />
      ) : null}
      ...
      {/* Empty-state: an active workspace with no open session shows the agent
          ...
          Specification runs are displayed in Run Center above; the normal
          launcher remains an independent terminal action. */}
