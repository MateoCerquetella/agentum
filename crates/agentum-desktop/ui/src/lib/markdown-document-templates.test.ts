...
describe('markdown document templates', () => {
  ...
  it('discovers markdown files under .agentum/templates and skips unsafe entries', async () => {
    const readDir = stubReadDir({
      '/repo/.agentum/templates': [
        entry('daily-note.md'),
        entry('scratch.txt'),
      ...
      '/repo/.agentum/templates/nested': [entry('meeting.markdown'), entry('brief.mdx')]
    })
    ...
    await expect(
      ...
      {
        id: '.agentum/templates/nested/brief.mdx',
        name: 'Brief',
        filePath: '/repo/.agentum/templates/nested/brief.mdx',
        relativePath: '.agentum/templates/nested/brief.mdx',
        templateRelativePath: 'nested/brief.mdx',
        basename: 'brief.mdx'
      },
      {
        id: '.agentum/templates/daily-note.md',
        name: 'Daily note',
        filePath: '/repo/.agentum/templates/daily-note.md',
        relativePath: '.agentum/templates/daily-note.md',
        templateRelativePath: 'daily-note.md',
        basename: 'daily-note.md'
      },
      {
        id: '.agentum/templates/nested/meeting.markdown',
        name: 'Meeting',
        filePath: '/repo/.agentum/templates/nested/meeting.markdown',
        relativePath: '.agentum/templates/nested/meeting.markdown',
    ...
    expect(readDir).toHaveBeenCalledWith({
      dirPath: '/repo/.agentum/templates',
      connectionId: 'conn-1'
    })
  })
  ...
  it('keeps Windows file paths native while exposing root-relative template paths', async () => {
    stubReadDir({
      'C:\\repo\\.agentum\\templates': [entry('daily.md')]
    })
    ...
    await expect(
      ...
      {
        id: '.agentum/templates/daily.md',
        name: 'Daily',
        filePath: 'C:\\repo\\.agentum\\templates\\daily.md',
        relativePath: '.agentum/templates/daily.md',
