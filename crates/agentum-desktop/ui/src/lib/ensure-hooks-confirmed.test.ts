...
function createTestState(overrides?: Partial<AppState>): {
  ...
  const trust: PersistedTrustedAgentumHooks = {}
  const state = {
    trustedAgentumHooks: trust,
...
describe('ensureHooksConfirmed', () => {
  ...
  it('short-circuits to run when the persisted content hash matches the current script', async () => {
    ...
    const hash = await hashAgentumHookScript(script)
    state.trustedAgentumHooks['repo-1'] = {
  ...
  it('re-prompts when the script content differs from the persisted hash', async () => {
    const { state, pending } = createTestState()
    const staleHash = await hashAgentumHookScript('old script')
    state.trustedAgentumHooks['repo-1'] = {
    ...
    // agentum.yaml changed, not because they've never approved this hook.
  ...
  it('includes default tab commands in the setup trust prompt', async () => {
    ...
    expect(pending[0].data.contentHash).toBe(await hashAgentumHookScript(expectedContent))
  ...
  it('does not prompt for agentum.yaml when the repo uses local commands only', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('does not prompt for agentum.yaml when local commands are the implicit default', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('opens a modal with the computed content hash and resolves with the user decision', async () => {
    ...
    expect(pending[0].data).toMatchObject({
      ...
      contentHash: await hashAgentumHookScript('pnpm install'),
