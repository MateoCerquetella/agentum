 ...
 * global shortcut) transfers focus out of Agentum and back, so focus-in is a
 * reliable proxy. The only missed case is a layout change triggered by a
 * key pressed while Agentum is focused (e.g. a Karabiner rule), which is
...
function _resetOptionAsAltProbeForTests(): void {
  _singleton?.dispose()
  _singleton = null
}
