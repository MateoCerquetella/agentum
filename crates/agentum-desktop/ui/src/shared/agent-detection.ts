...
//     keyword land right after a path separator. Agentum is a cross-platform
...
export function normalizeTerminalTitle(title: string): string {
  ...
  // churn while preserving the working/idle transition Agentum keys off.
...
export function getAgentLabel(title: string): string | null {
  ...
  // (verified against the 2026.04.17 release) — Agentum synthesizes the same
...
export function detectAgentStatusFromTitle(title: string): AgentStatus | null {
  ...
  // action required") is either an Agentum-synthesized working/permission title
