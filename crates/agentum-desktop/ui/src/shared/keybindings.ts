...
export type TerminalShortcutPolicy = 'agentum-first' | 'terminal-first'
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'agentum-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: Agentum-first preserves existing app shortcut behavior inside terminals.
  // Terminal-first is the explicit escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'agentum-first') {
    return true
  }
