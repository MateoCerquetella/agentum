...
function App(): React.JSX.Element {
  ...
  const setAppRootNode = useCallback(
    (node: HTMLDivElement | null): void => {
      // Why: this best-effort App chrome cleanup shares the App root lifetime.
      if (!node) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // agentum-data.json, silently erasing the user's saved tabs. The fix is
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const notifyTerminalCapture = (actionId: KeybindingActionId): void => {
        if (
          context !== 'terminal' ||
          (settings?.terminalShortcutPolicy ?? 'agentum-first') !== 'agentum-first'
        ) {
          return
        }
                ...
                <div className="flex flex-row flex-1 min-h-0 overflow-hidden">
                  {showSidebar ? (
                    workspaceActive ? (
                     ...
                     (traffic lights, sidebar toggle, "Agentum" title, agent badge)
