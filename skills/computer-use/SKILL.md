---
name: computer-use
description: Use agentum's computer-use CLI to inspect and control local desktop apps through accessibility trees, screenshots, and safe UI actions. Use when an agent needs to list desktop apps, get an app state, read visible UI, click, type, press keys, scroll, drag, set values, or perform app accessibility actions. Triggers include "computer use", "agentum computer", "list apps", "get app state", "read Spotify", "read Slack", "click app", "type text", "press key", "set value", "scroll app", "drag app", and desktop app interaction tasks.
---

# Computer Use

Use this skill when the task should operate through agentum's desktop computer-use surface rather than native Codex computer tools, raw AppleScript, ad hoc screenshots, or direct app internals.

## Preconditions

- Prefer the public `agentum computer ...` command.
- In this agentum worktree, use `./config/scripts/agentum-dev computer ...` when testing the local dev runtime.
...
```bash
agentum status --json
agentum computer capabilities --json
```

For local development against this worktree:

```bash
./config/scripts/agentum-dev status --json
```
...
```bash
agentum computer list-apps --json
```

2. Get a fresh state for the target app:

```bash
agentum computer get-app-state --app com.spotify.client --json
```
...
```bash
agentum computer click --app com.spotify.client --element-index 42 --json
```
...
```bash
agentum computer get-app-state --app com.microsoft.edgemac --json
agentum computer get-app-state --app com.spotify.client --json
```
...
```bash
agentum computer get-app-state --app Spotify --json
```

Use `pid:<number>` only when bundle ID or name matching is ambiguous:

```bash
agentum computer get-app-state --app pid:12345 --json
```

## Commands

```bash
agentum computer permissions --json
agentum computer capabilities --json
agentum computer list-apps --json
agentum computer list-windows --app <app> --json
agentum computer get-app-state --app <app> --json
agentum computer click --app <app> --element-index <index> --json
agentum computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
agentum computer set-value --app <app> --element-index <index> --value "text" --json
agentum computer type-text --app <app> --text "text" --json
agentum computer press-key --app <app> --key Return --json
agentum computer hotkey --app <app> --key CmdOrCtrl+A --json
agentum computer paste-text --app <app> --text "text" --json
agentum computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
agentum computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | agentum computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```bash
agentum computer get-app-state --app <app> --restore-window --json
```
...
```bash
agentum computer get-app-state --app com.microsoft.edgemac --json
agentum computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
agentum computer press-key --app com.microsoft.edgemac --key Return --json
agentum computer get-app-state --app com.microsoft.edgemac --json
```
...
- Permission errors: the user needs to grant Accessibility or Screen Recording to `agentum Computer Use`. Run `agentum computer permissions --json`, use the setup UI, then retry `agentum computer get-app-state --app <bundle> --json`.
