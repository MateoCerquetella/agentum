...
name: orchestration
description: >-
  Use for agentum agent-to-agent coordination: send/ask/reply between agent
  ...
  Use `agentum-cli` instead for terminal control, shell commands, browser
  automation, worktree management, and reading or waiting on terminals.
---

# agentum Inter-Agent Orchestration

Use this skill when the task involves coordinating multiple coding agents through agentum's orchestration system. For basic terminal and worktree management, use the `agentum-cli` skill instead.
...
Use `agentum-cli` instead for ordinary terminal control, shell commands, browser automation, worktree management, or reading/waiting on terminals.

## Preconditions

- agentum must be running (`agentum status --json` should return `runtime: true`).
- The `agentum` CLI must be on PATH (installed via Settings > Browser > Enable agentum CLI).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- All `agentum orchestration` commands are RPC calls to the running agentum runtime — they require an active agentum session.
...
When the handoff type is unclear, inspect `agentum orchestration task-list --json`, `agentum orchestration dispatch-show`, and `agentum terminal list --json` for the task, dispatch, and handle ownership before sending lifecycle messages. If you still cannot tell whether the remote handle owns an active dispatch, ask the current owner instead of silently completing a task into an unrelated workstream.
...
```bash
agentum orchestration send --to <handle|@group> --subject <text> [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
agentum orchestration check [--terminal <handle>] [--unread] [--types <type,...>] [--inject] [--wait] [--timeout-ms <n>] [--json]
agentum orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
agentum orchestration inbox [--limit <n>] [--json]
```

Why: `--from` auto-resolves via the `AGENTUM_TERMINAL_HANDLE` environment variable injected into every agentum-managed terminal. Omit it unless impersonating another terminal.
...
```bash
agentum orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
agentum orchestration task-list [--status <status>] [--ready] [--json]
agentum orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
```
...
```bash
agentum orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
agentum orchestration dispatch-show --task <task_id> [--json]
```

Why: `--inject` sends a preamble that teaches the agent how to use `agentum orchestration send --type worker_done` to report completion. All agents have `agentum` on PATH and can execute shell commands. The preamble maximizes structured feedback but the system works without it (coordinator falls back to idle detection + output reading).
...
```bash
agentum orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
agentum orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
agentum orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
```bash
agentum orchestration run --spec <text> [--from <handle>] [--poll-interval-ms <n>] [--max-concurrent <n>] [--worktree <selector>] [--json]
agentum orchestration run-stop [--json]
```

Why: `run` returns immediately with a run ID. The coordinator loop runs in the background inside the agentum runtime. Query progress via `agentum orchestration task-list`. Only one coordinator can run at a time.
...
```bash
agentum orchestration reset [--all] [--tasks] [--messages] [--json]
```
...
Coordinators need these terminal commands to spawn agents, monitor progress, and read output. Full terminal documentation lives in the `agentum-cli` skill — this is the subset required for orchestration workflows.

```bash
agentum terminal list [--worktree <selector>] [--json]
agentum terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
agentum terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
agentum terminal read [--terminal <handle>] [--json]
agentum terminal send [--terminal <handle>] --text <text> [--enter] [--json]
agentum terminal wait [--terminal <handle>] --for <exit|tui-idle> [--timeout-ms <n>] [--json]
agentum terminal show --terminal <handle> [--json]
agentum terminal stop [--terminal <handle>] [--json]
agentum terminal close [--terminal <handle>] [--json]
```

Why: `--terminal` is optional for most commands. When omitted, agentum auto-resolves to the active terminal in the current worktree.

Why: `--command "claude"` launches Claude Code in the new terminal. In local agentum sessions, `--command "codex"` launches Codex through agentum's visible terminal path automatically so Codex does not start as a headless/background PTY. After creating a `--command` terminal, use `terminal wait --for tui-idle` to wait for the agent to boot before dispatching.
...
Why: terminal handles are runtime-scoped. If agentum restarts, handles go stale. Re-acquire with `terminal list`.
...
- Use `agentum orchestration check` to read incoming messages from the coordinator or other agents. Messages are delivered automatically when you go idle, but you can also poll explicitly.
- Treat `agentum orchestration` commands the same way you treat `git` or `npm` — they are CLI tools available in your shell.
- The coordinator uses `agentum orchestration task-list --ready` as its external memory. Prefer querying orchestration state over tracking it in your context window.
...
- Terminal handles are ephemeral and runtime-scoped. If agentum restarts mid-workflow, all handles go stale. Re-acquire them with `terminal list` before continuing.
...
agentum terminal create --worktree active --title "worker-1" --command "claude" --json
# → handle: term_abc123

# 2. Wait for Claude Code to boot (tui-idle fires when the prompt appears)
agentum terminal wait --terminal term_abc123 --for tui-idle --timeout-ms 60000 --json

# 3. Create and dispatch a task with preamble injection
agentum orchestration task-create --spec "Fix the login button CSS" --json
# → id: task_def456
agentum orchestration dispatch --task task_def456 --to term_abc123 --inject --json

# 4. Block until the worker reports back (no sleep loops needed)
agentum orchestration check --wait --types worker_done,escalation --timeout-ms 300000 --json
# → returns immediately when worker sends worker_done

# 5. If --wait timed out with no messages, fall back to idle detection
agentum terminal wait --terminal term_abc123 --for tui-idle --timeout-ms 60000 --json
agentum terminal read --terminal term_abc123 --json
```
