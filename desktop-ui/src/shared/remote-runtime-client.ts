...
export async function sendRemoteRuntimeRequest<TResult>(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): void => {
      ...
      // Why: the settled one-shot no longer needs Agentum callbacks, but a ws
    ...
    function onMessage(data: WebSocket.RawData, isBinary: boolean): void {
      ...
      if (isBinary) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            ...
            'Remote Agentum runtime returned an unexpected binary frame.'
          )
        })
        return
      }
      ...
      if (plaintext === null) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an undecryptable frame.'
          )
        })
        return
      }
    ...
    function handleReadyFrame(frame: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an invalid E2EE handshake frame.'
          )
        })
        return
      }
      if (
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an unexpected E2EE handshake frame.'
          )
        })
        return
      }
    ...
    function handleAuthenticatedFrame(plaintext: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an invalid E2EE auth frame.'
          )
        })
        return
      }
      ...
      if (type !== 'e2ee_authenticated') {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            code,
            'Remote Agentum runtime rejected the pairing token.'
          )
        })
        return
      }
    ...
    function handleRpcFrame(plaintext: string): void {
      ...
      try {
        ...
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an invalid response frame.'
          )
        })
        return
      }
      ...
      if (response.id !== requestId) {
        finish({
          ...
          error: new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned a mismatched response id.'
          )
        })
        return
      }
...
export async function subscribeRemoteRuntimeRequest<TResult>(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanupSocketListeners = (): WebSocket | null => {
      ...
      socket.off('message', onMessage)
      ws = null
      // Why: startup failures detach Agentum callbacks before closing the ws,
    ...
    const timeout = setTimeout(() => {
      fail(
        new RemoteRuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the remote Agentum runtime subscription to start.'
        )
      )
    }, timeoutMs)
      ...
      if (type !== 'e2ee_authenticated') {
        const code =
          ...
          (authenticated as { error?: { code?: unknown } }).error?.code === 'unauthorized'
            ? 'unauthorized'
            : 'invalid_runtime_response'
        fail(new RemoteRuntimeClientError(code, 'Remote Agentum runtime rejected the pairing token.'))
        return
      }
      state = 'ready'
      ws?.send(
    ...
    function handleBinaryFrame(frame: Uint8Array<ArrayBufferLike>): void {
      if (state !== 'ready') {
        fail(
          new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned binary data before authentication.'
          )
        )
        return
      }
      ...
      if (plaintext === null) {
        fail(
          new RemoteRuntimeClientError(
            'invalid_runtime_response',
            'Remote Agentum runtime returned an undecryptable binary frame.'
          )
        )
        return
      }
