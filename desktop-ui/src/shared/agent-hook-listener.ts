/* eslint-disable max-lines -- Why: this module is the canonical, transport-
   ...
   between Agentum's main process and the relay. Splitting by line count would
...
// docs/design/agent-status-over-ssh.md §3 ("relay normalizes; Agentum routes").
...
import { AGENTUM_HOOK_PROTOCOL_VERSION } from './agent-hook-types'
 ...
 *  Exported so non-HTTP ingest paths (e.g. Agentum's `ingestRemote`) can apply
 ...
 *  (last prompt, last tool snapshot, last status replay). Both Agentum's main
...
export function warnOnHookEnvOrVersionMismatch(
  ...
  if (
    version &&
    version !== AGENTUM_HOOK_PROTOCOL_VERSION &&
    !state.warnedVersions.has(version) &&
    state.warnedVersions.size < MAX_WARNED_KEYS
  ) {
    state.warnedVersions.add(version)
    console.warn(
      `[agent-hooks] received hook v${version}; server expects v${AGENTUM_HOOK_PROTOCOL_VERSION}. ` +
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Agentum install.'
      )
    }
  }
}
...
export type AgentHookEventPayload = {
   ...
   *  Stamped only on the remote-ingest path (Agentum's `ingestRemote`); the
...
export function normalizeHookPayload(
  ...
  // it null; the relay forwards null on the wire and Agentum's `ingestRemote`
...
export function writeEndpointFile(
  ...
  const valuesToWrite: [string, string][] = [
    ['AGENTUM_AGENT_HOOK_PORT', String(fields.port)],
    ['AGENTUM_AGENT_HOOK_TOKEN', fields.token],
    ['AGENTUM_AGENT_HOOK_ENV', fields.env],
    ['AGENTUM_AGENT_HOOK_VERSION', fields.version]
  ]
  ...
  try {
    ...
    // not leak the existence of this Agentum/relay install to other local users.
