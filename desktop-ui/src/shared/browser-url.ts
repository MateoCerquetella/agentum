...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === AGENTUM_BROWSER_BLANK_URL) {
    return AGENTUM_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === AGENTUM_BROWSER_BLANK_URL) {
    return null
  }
