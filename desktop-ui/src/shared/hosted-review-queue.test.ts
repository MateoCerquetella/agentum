...
function baseSummary(overrides: Partial<HostedReviewQueueSummary> = {}): HostedReviewQueueSummary {
  return {
    identity: { provider: 'github', host: 'github.com', owner: 'acme', repo: 'agentum', number: 42 },
...
describe('classifyHostedReview', () => {
  it('classifies mine/requested/agent/teammate', () => {
    ...
    expect(
      classifyHostedReview(baseSummary({ author: { login: 'agentum-ci' } }), {
        agentAuthorLogins: ['agentum-ci']
      }).state
    ).toBe('agent')
