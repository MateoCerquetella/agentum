...
// the relay sends to Agentum. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Agentum's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Agentum routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Agentum's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Agentum's
//   local handle on an `ssh2` connection, not a wire identity. Agentum stamps the
...
//   POST body so Agentum's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Agentum can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Agentum. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Agentum's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Agentum can
  ...
  /** True when the relay is replaying its cache after Agentum reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Agentum's warn-once
   ...
   *  Agentum's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Agentum issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Agentum issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Agentum and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const AGENTUM_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'AGENTUM_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[AGENTUM_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
