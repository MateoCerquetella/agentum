...
export function parseAgentHookEndpointFile(contents: string): AgentHookEndpoint {
  ...
  if (
    !values.AGENTUM_AGENT_HOOK_PORT ||
    !values.AGENTUM_AGENT_HOOK_TOKEN ||
    !values.AGENTUM_AGENT_HOOK_ENV ||
    !values.AGENTUM_AGENT_HOOK_VERSION
  ) {
    throw new Error('Agent hook endpoint file is missing required fields')
  }
  return {
    port: values.AGENTUM_AGENT_HOOK_PORT,
    token: values.AGENTUM_AGENT_HOOK_TOKEN,
    env: values.AGENTUM_AGENT_HOOK_ENV,
    version: values.AGENTUM_AGENT_HOOK_VERSION
  }
}
