  ...
  const ensureRemotePage = useCallback(
    async (token: RemoteBrowserOperationToken): Promise<string | null> => {
      ...
      const createRemotePage = async (): Promise<string | null> => {
        ...
        const initialUrl =
          currentUrl === AGENTUM_BROWSER_BLANK_URL ? 'about:blank' : currentUrl || 'about:blank'
...
function BrowserPagePane({
  ...
  const sessionProfile = sessionProfileId
    ...
    : null
  const webviewPartition = sessionProfile?.partition ?? AGENTUM_BROWSER_PARTITION
  ...
  useEffect(() => {
    return window.api.browser.onDownloadRequested((event) => {
      ...
      // triggered the save prompt before Agentum asks main to choose a path.
      setDownloadState({
        ...event,
        receivedBytes: 0,
        status: 'requested'
  ...
  useEffect(() => {
    ...
    window.addEventListener(AGENTUM_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(AGENTUM_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
  }, [browserTab.id, focusAddressBarNow, focusWebviewNow, isActive])
    ...
    (webview: Electron.WebviewTag): void => {
      try {
        onUpdatePageStateRef.current(browserTab.id, {
          ...
          // Only explicit navigation/load events should drive Agentum's loading UI.
          canGoBack: webview.canGoBack(),
          canGoForward: webview.canGoForward()
        })
      } catch {
        // Why: Electron only exposes these getters after the guest fully
        // attaches. Ignoring the transient failure avoids crashing Agentum while
  ...
  useEffect(() => {
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (activeLoadFailure) {
        ...
        if (normalizedAttemptedUrl === normalizedCurrentUrl) {
          ...
          // navigation falls back to a blank Chromium surface even though Agentum
      ...
      if (keepAddressBarFocusRef.current && currentUrl === AGENTUM_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    const handleFailLoad = (event: {
      ...
      if (event.errorCode === -3) {
        ...
        // when the replacement navigation succeeds. Ignore that noise so Agentum
    ...
    if (needsInitialNavigation) {
      ...
      // Agentum never misses the initial did-fail-load signal for a new tab.
      // Only non-blank initial tabs should light up Agentum's loading indicator;
      // reclaiming/activating a parked about:blank tab is not a meaningful
      // navigation and should not flash the tab-loading dot.
      const initialUrl =
        normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? AGENTUM_BROWSER_BLANK_URL
      trackNextLoadingEventRef.current = initialUrl !== AGENTUM_BROWSER_BLANK_URL
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are Agentum-driven navigations (address bar,
      ...
      trackNextLoadingEventRef.current = normalizedUrl !== AGENTUM_BROWSER_BLANK_URL
      lastKnownWebviewUrlRef.current = normalizedUrl
      webview.src = normalizedUrl
      if (normalizedUrl !== AGENTUM_BROWSER_BLANK_URL) {
        keepAddressBarFocusRef.current = false
        if (document.activeElement === addressBarInputRef.current) {
          focusWebviewNow()
        }
  ...
  useEffect(() => {
    ...
    // Polling only while the active tab is "loading" gives Agentum a last-resort
  ...
  const navigateToUrl = useCallback(
    (url: string): void => {
      const navigateBrowserUrl = (targetUrl: string): void => {
        ...
        trackNextLoadingEventRef.current = targetUrl !== AGENTUM_BROWSER_BLANK_URL
        lastKnownWebviewUrlRef.current =
          normalizeBrowserNavigationUrl(browserModelUrl) ?? browserModelUrl
        webview.src = targetUrl
        if (targetUrl !== AGENTUM_BROWSER_BLANK_URL) {
          focusWebviewNow()
        }
      }
  ...
  const isBlankTab = browserTab.url === 'about:blank' || browserTab.url === AGENTUM_BROWSER_BLANK_URL
              ...
              <div className="mt-5 flex items-center gap-2">
                ...
                {externalUrl ? (
                  <Button
                    ...
                    onClick={() => {
                      // Why: page failures inside Agentum can still be recoverable
