...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Agentum denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    expect(
      formatPopupNotice({
        browserPageId: 'browser-1',
        origin: 'https://example.com',
        action: 'opened-in-agentum'
      })
    ).toBe('https://example.com opened a new page in Agentum.')
    ...
    ).toBe('A site tried to open a popup Agentum does not support here.')
  })
