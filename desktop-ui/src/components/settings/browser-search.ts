...
export function getBrowserLinkRoutingDescription(platform: BrowserShortcutPlatform): string {
  return `Open http(s) links in Agentum's built-in browser — from the terminal, markdown, and the editor. ${getBrowserLinkRoutingShortcutLabel(platform)} always uses your system browser.`
}
...
export function getBrowserPaneSearchEntries(
  ...
  return [
    ...
    },
    {
      ...
      description:
        'Import cookies from Chrome, Edge, or other browsers to use existing logins inside Agentum.',
      keywords: [
        'browser',
        'cookies',
        'session',
