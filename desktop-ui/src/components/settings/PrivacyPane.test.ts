...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for AGENTUM_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'agentum_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps agentum_disabled to AGENTUM_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('agentum_disabled')).toBe('AGENTUM_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names AGENTUM_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'agentum_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'agentum_disabled' })
  })
