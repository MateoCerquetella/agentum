...
import { AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT } from './editor-autosave'
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(AGENTUM_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
