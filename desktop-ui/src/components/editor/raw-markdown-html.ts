...
const INLINE_PLACEHOLDER_PREFIX = '[[AGENTUM_RAW_HTML_INLINE:'
const BLOCK_PLACEHOLDER_PREFIX = '[[AGENTUM_RAW_HTML_BLOCK:'
const DOC_LINK_PLACEHOLDER_PREFIX = '[[AGENTUM_DOC_LINK:'
...
export function encodeRawMarkdownHtmlForRichEditor(content: string): string {
  ...
  while (index < content.length) {
    ...
    // skipped by the guards above. The [[AGENTUM_ prefix check prevents re-encoding
    // sibling placeholders that were already emitted earlier in this pass.
    if (
      content[index] === '[' &&
      content[index + 1] === '[' &&
      !content.startsWith('[[AGENTUM_', index) &&
...
export const RawMarkdownHtmlInline = Node.create({
  ...
  // while still letting Agentum round-trip the raw markup verbatim.
