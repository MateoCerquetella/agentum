...
describe('createTerminalGitHubPRLinkDetector', () => {
  it('extracts GitHub pull request URLs from terminal output', () => {
    ...
    expect(observe('Created https://github.com/acme/agentum/pull/42\r\n')).toEqual([
      {
  ...
  it('waits for a boundary when the URL is split across PTY chunks', () => {
    ...
    expect(observe('https://github.com/acme/agentum/pull/4')).toEqual([])
    expect(observe('2\r\n')).toEqual([
      {
        url: 'https://github.com/acme/agentum/pull/42',
        slug: { owner: 'acme', repo: 'agentum' },
  ...
  it('detects a URL split inside the GitHub prefix', () => {
    ...
    expect(observe('ub.com/acme/agentum/pull/42\n')).toEqual([
      {
        url: 'https://github.com/acme/agentum/pull/42',
        slug: { owner: 'acme', repo: 'agentum' },
        number: 42
  ...
  it('trims terminal punctuation around printed URLs', () => {
    ...
    expect(observe('Opened (https://github.com/acme/agentum/pull/42).\n')[0]?.url).toBe(
      'https://github.com/acme/agentum/pull/42'
    )
  })
  ...
  it('does not repeat the same PR URL from overlapping carry text', () => {
    ...
    expect(observe('https://github.com/acme/agentum/pull/42\n')).toHaveLength(1)
  ...
  it('ignores non-PR and non-GitHub links', () => {
    ...
    expect(
      observe(
        'https://github.com/acme/agentum/issues/42 https://github.example.com/acme/agentum/pull/42\n'
      )
    ).toEqual([])
  })
})
